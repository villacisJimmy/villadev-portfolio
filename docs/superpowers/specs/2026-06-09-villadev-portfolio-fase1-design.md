...
interface Service { id: string; index: string; title: LocalizedString; description: LocalizedString; tags: string[]; }
interface Skill { id: string; category: LocalizedString; description: LocalizedString; }
```
...
**Jobs adicionales en push a `main`:**
7. `docker-build-push` — `docker build` + push a GHCR con tags `sha-<short>` y `latest`. Build con `BuildKit` y caché.
8. `deploy` — SSH al VPS (`appleboy/ssh-action`) ejecuta `/srv/villadev/deploy.sh` que hace `docker compose pull && docker compose up -d --remove-orphans && docker image prune -af`.
